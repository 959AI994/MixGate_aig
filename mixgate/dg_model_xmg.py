...
class Model(nn.Module):
    ...
    def forward(self, G):
        ...
        and_mask = G.xmg_gate.squeeze(1) == 1 #判断门的种类
        not_mask = G.xmg_gate.squeeze(1) == 2
        xor_mask = G.xmg_gate.squeeze(1) == 3     
        ...
        for _ in range(self.num_rounds):
            for level in range(1, num_layers_f):
                # forward layer
                layer_mask = G.xmg_forward_level == level #将目标层级不mask掉，G.forward_level是各节点的层级信息
                # print("G.level =", G.forward_level)
                # print("layer_mask =", layer_mask)

                # AND Gate
                l_and_node = G.xmg_forward_index[layer_mask & and_mask]#将目标层级的and节点显示出来
                ...
                l_xor_node = G.xmg_forward_index[layer_mask & xor_mask]#将目标层级的and节点显示出来
                # print("l_and_node =", l_and_node)
